...
| `UserSchemaClass` | `infrastructure/document/entities/user.schema.ts` | Mongoose schema: `email` (unique), `password` (`@Exclude` `toPlainOnly` — **declared but not enforced at runtime; see § Known Limitations**), embedded `Preferences`, `favoriteRecipes[]`, `recentSearches[]`, `deletedAt`. |
...
so routes resolve at `/api/users/*`. The `version: '1'` argument is **inert at
runtime** because `main.ts` does not call `app.enableVersioning()` (Source:
`backend/src/main.ts:L10-L35`); the routes therefore carry no `/v1/` segment and
`/api/v1/users/*` returns 404. The Swagger surface is published at `/docs`.
...
> ⚠️ **Route versioning is inert** — the controller declares `@Controller({ path: 'users', version: '1' })`, but `main.ts` never calls `app.enableVersioning()` (Source: `backend/src/main.ts:L10-L35`), so the version argument is a no-op. Routes resolve at `/api/users/*` (not `/api/v1/users/*`); `/api/v1/users/*` returns 404. See [PRODUCTION_READINESS.md](../../../PRODUCTION_READINESS.md) § Build & Runtime.

> ⚠️ **`@Exclude` is not enforced at runtime** — the `password` field carries `@Exclude({ toPlainOnly: true })`, but no `ClassSerializerInterceptor` is registered (neither `useGlobalInterceptors` in `main.ts` nor an `APP_INTERCEPTOR` provider in `app.module.ts`), so class-transformer never strips it. The bcrypt password hash is therefore returned in `GET /api/auth/me` and `GET /api/users/me` responses. See [PRODUCTION_READINESS.md](../../../PRODUCTION_READINESS.md) § Security Hardening and [DATA_MODEL.md](../../../DATA_MODEL.md) § User.
