...
- **`PantryService`** — the application-layer coordinator that delegates to the
  repository contract.
  Source: backend/src/pantry/pantry.service.ts:L15-L17.
...
- **`PantryIngridientDocumentRepository`** — the Mongoose-backed implementation
  of that contract.
  Source: backend/src/pantry/infrastructure/document/repositories/pantryIngridient.repository.ts:L16-L21.
...
- **DTOs** — `create-pantry-ingridient.dto.ts`, `update-pantry-ingridient.dto.ts`,
  ...
  Source: backend/src/pantry/dto/create-pantry-ingridient.dto.ts:L11,
  backend/src/pantry/dto/update-pantry-ingridient.dto.ts:L7-L9,
  backend/src/pantry/dto/query-pantry-ingridient.dto.ts:L28.
...
| Field | Type | Notes |
|-------|------|-------|
| `ingridient` | ObjectId | Reference to `IngridientSchemaClass` (the Ingridient catalog). Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L17-L18 |
| `quantity` | number | Amount of the ingredient on hand. Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L20-L21 |
| `userId` | string | Owner of the entry; all queries scope on this value. Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L23-L24 |
| `unit` | string | Unit of measure for the quantity. Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L26-L27 |
| `expirationDate` | Date (optional) | When the item expires. Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L29-L30 |
| `location` | enum | Storage location; see the enum table below. Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L32-L33 |
| `createdAt` | Date | Creation timestamp (`default: now`). Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L35-L36 |
| `updatedAt` | Date | Last-update timestamp (`default: now`). Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L38-L39 |
| `deletedAt` | Date (optional) | Soft-delete marker field (see Known limitations). Source: backend/src/pantry/infrastructure/document/entities/pantryIngridient.schema.ts:L41-L42 |
...
backend/src/pantry/domain/pantryIngridient.ts:L10.
...
|---------------|-------------|--------|
| `POST /api/pantry` | Create a pantry entry; `userId` is taken from the authenticated request, not the body. | backend/src/pantry/pantry.controller.ts:L36-L44 (userId at L42) |
| `GET /api/pantry` | List entries; pagination is capped at 50 items per page (default 10); the filter is scoped to `userId`. | backend/src/pantry/pantry.controller.ts:L55-L56, L52, L61 |
| `GET /api/pantry/:id` | Fetch a single entry by id. | backend/src/pantry/pantry.controller.ts:L72-L83 |
| `PATCH /api/pantry/:id` | Partial update; throws `422` when the record does not exist. | backend/src/pantry/pantry.controller.ts:L85-L97, backend/src/pantry/pantry.service.ts:L66-L75 |
| `DELETE /api/pantry/:id` | Delete an entry; responds `204 No Content`. | backend/src/pantry/pantry.controller.ts:L99-L108 |
...
Source: backend/src/pantry/infrastructure/document/document-persistence.module.ts:L12-L17.
...
`PantryService`.
Source: backend/src/pantry/pantry.controller.ts:L34.
The service calls the abstract `PantryRepository`.
Source: backend/src/pantry/pantry.service.ts:L17.
...
- **Mapper pattern** — `PantryIngridientMapper.toDomain` and `toPersistence`
  ...
  Source: backend/src/pantry/infrastructure/document/mappers/pantryIngridient.mapper.ts:L7,
...
- **`userId` scoping** — the owner id is injected from the JWT request rather
  ...
  Source: backend/src/pantry/pantry.controller.ts:L42, L52.
- **Pagination clamping** — the list `limit` is capped at 50.
  Source: backend/src/pantry/pantry.controller.ts:L55-L56.
