...
| `Endpoints.refreshToken` | `$apiBaseUrl/auth/refresh` | Used by `_refreshDio` on 401/`tokenExpired` |
| `Endpoints.login` | `$apiBaseUrl/auth/email/login` | Email login |
| `Endpoints.signup` | `$apiBaseUrl/auth/email/register` | Email register |
| `Endpoints.logout` | `$apiBaseUrl/auth/logout` | Logout |
| `Endpoints.profile` | `$apiBaseUrl/auth/me` | Current user profile |
| `Endpoints.updateProfile` | `$apiBaseUrl/users` | Update user profile |
| `Endpoints.recipe` | `$apiBaseUrl/recipe` | Recipe module root |
| `Endpoints.ingredient` | `$apiBaseUrl/ingredient` | Ingredient module root |
| `Endpoints.ingredientCreationData` | `$ingredient/creation-data` | Category/unit reference data |
| `Endpoints.pantry` | `$apiBaseUrl/pantry` | Pantry module root |
| `Endpoints.ai` | `$apiBaseUrl/ai` | AI vision module root |
...
| `Endpoints.refreshToken` | `$apiBaseUrl/auth/refresh` | `mobile/lib/core/constants/endpoints.dart:L11` | Triggered on 401/419 |
...
> 🚧 **Single-flight refresh** — add a token-lock semaphore to `DioClient` so concurrent `401` responses coalesce into a single `/auth/refresh` call. Maps to [../../../PRODUCTION_READINESS.md](../../../PRODUCTION_READINESS.md) § Security Hardening.
