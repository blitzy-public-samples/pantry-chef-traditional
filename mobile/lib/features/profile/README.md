...
(`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L18`).
...
- `ProfileAvatar` — a circular avatar widget
  (`Source: mobile/lib/features/profile/presentation/widgets/profile_avatar.dart:L10`).
...
- `ProfileMain` — the profile home screen
  (`Source: mobile/lib/features/profile/presentation/widgets/screens/profile_main.dart:L21`).
- `FavoriteRecipes` — the favorites list screen
  (`Source: mobile/lib/features/profile/presentation/widgets/screens/favorite_recipes.dart:L21`).
- `PreferencesScreen` — the preferences screen (see Known limitations / gaps)
  (`Source: mobile/lib/features/profile/presentation/widgets/screens/preferences.dart:L10`).
...
|-------|------|-------|--------|
| `id` | `String` | User identifier | `Source: mobile/lib/features/profile/domain/models/profile.dart:L48` |
| `email` | `String` | User email | `Source: mobile/lib/features/profile/domain/models/profile.dart:L49` |
| `favoriteRecipes` | `List<String>` | Favorite recipe identifiers | `Source: mobile/lib/features/profile/domain/models/profile.dart:L50` |
| `recentSearches` | `List<String>` | Recent search terms | `Source: mobile/lib/features/profile/domain/models/profile.dart:L51` |
| `preferences` | `Preferences` | `@JsonKey(toJson: Mappers.preferencesToJson)` | `Source: mobile/lib/features/profile/domain/models/profile.dart:L52` |
...
|--------|------|---------|--------|
| `GET` | `/api/auth/me` | Fetch the current user's profile | `Source: mobile/lib/core/constants/endpoints.dart:L38` (`ProfileApi.getProfile`, `Source: mobile/lib/features/profile/data/api/profile.api.dart:L24-L27`) |
| `PATCH` | `/api/users` | Update the profile (patch body omits null fields) | `Source: mobile/lib/core/constants/endpoints.dart:L40` (`ProfileApi.updateProfile`, `Source: mobile/lib/features/profile/data/api/profile.api.dart:L32-L34`) |
| `POST` | `/api/auth/logout` | Server-side logout | `Source: mobile/lib/core/constants/endpoints.dart:L36` (`ProfileApi.logout`, `Source: mobile/lib/features/profile/data/api/profile.api.dart:L38-L40`) |
...
- **API base URL.** The backend root comes from `EnvConfig.apiBaseUrl`, a
  ...
  (`Source: mobile/lib/env_config.dart:L19`). `ProfileApi` reaches the network
...
- **Persisted state.** Hydrated storage is built once during app startup —
  ...
  (`Source: mobile/lib/main.dart:L29-L31`). `ProfileBloc` opts into that storage
  by calling `hydrate()` in its constructor, which is what makes the profile and
  favorites survive restarts
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L21`).

Override the API base URL at build/run time with
`--dart-define API_BASE_URL=http://<host>:3000/api`
(`Source: mobile/lib/env_config.dart:L19`); see Local development below.
...
(`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L18-L85`),
...
(`Source: mobile/lib/core/constants/endpoints.dart:L36-L40`)._
...
- **Load profile.** `ProfileFetched` triggers `GetProfileUsecase`, which calls
  ...
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L23-L29`).
- **Toggle a favorite.** `FavoriteRecipesListUpdated` rebuilds the favorite-id
  ...
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L43-L73`).
  An empty favorites list short-circuits the fetch path
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L31-L41`).
- **Logout.** `Logout` runs `LogoutUsecase`, and `ProfileDataReseted` emits a
  fresh `ProfileState`
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L75-L84`).
...
- **BLoC with persisted state.** `ProfileBloc` extends
  ...
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L18`,
  `Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L89-L97`,
  `Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L100-L106`).
...
(`Source: mobile/lib/features/profile/presentation/widgets/screens/preferences.dart:L14-L24`).
