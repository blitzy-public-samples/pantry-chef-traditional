...
(`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L14`).
The three domain operations are encapsulated as use cases:
`GetProfileUsecase`
(`Source: mobile/lib/features/profile/domain/usecases/get_profile.usecase.dart:L6`),
`FavoriteRecipesUpdateUsecase`
(`Source: mobile/lib/features/profile/domain/usecases/favorite_recipes_update.usecase.dart:L9`),
and `LogoutUsecase`
(`Source: mobile/lib/features/profile/domain/usecases/logout.usecase.dart:L15`).
...
- `ProfileEvent` and its events — the `sealed` event hierarchy
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_event.dart:L3`).
- `ProfileState` — the immutable state with `userProfile` and `favoriteRecipes`
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_state.dart:L3`).
...
- `ProfileRepository` — the abstract repository interface, correctly spelled
  (`Source: mobile/lib/features/profile/domain/repositories/profile.repository.dart:L3`).
- `GetProfileUsecase`
  (`Source: mobile/lib/features/profile/domain/usecases/get_profile.usecase.dart:L6`),
  `FavoriteRecipesUpdateUsecase`
  (`Source: mobile/lib/features/profile/domain/usecases/favorite_recipes_update.usecase.dart:L9`),
...
- `index.dart` barrel — re-exports only `get_profile.usecase.dart` and
  `logout.usecase.dart`
  (`Source: mobile/lib/features/profile/domain/usecases/index.dart:L1-L2`).

**Data — api, repository implementation, DTOs**

- `ProfileApi` — the Dio-backed HTTP client for the feature
  (`Source: mobile/lib/features/profile/data/api/profile.api.dart:L7`).
...
- `ProfileUpdateDto` — the patch-style update DTO
  (`Source: mobile/lib/features/profile/data/dto/profile_update.dto.dart:L7`).
- `FavoriteRecipesUpdateDto` — a plain (non-serialized) parameter object
  (`Source: mobile/lib/features/profile/data/dto/favorite_recipes_update.dto.dart:L1`).

**Presentation — widgets and screens**

- `ProfileAvatar` — a circular avatar widget
  (`Source: mobile/lib/features/profile/presentation/widgets/profile_avatar.dart:L4`).
- `ProfileNavigationItem` — a tappable navigation row
  (`Source: mobile/lib/features/profile/presentation/widgets/profile_navigation_item.dart:L4`).
- `ProfileMain` — the profile home screen
  (`Source: mobile/lib/features/profile/presentation/widgets/screens/profile_main.dart:L12`).
- `FavoriteRecipes` — the favorites list screen
  (`Source: mobile/lib/features/profile/presentation/widgets/screens/favorite_recipes.dart:L12`).
- `PreferencesScreen` — the preferences screen (see Known limitations / gaps)
  (`Source: mobile/lib/features/profile/presentation/widgets/screens/preferences.dart:L6`).
...
(`Source: mobile/lib/features/profile/data/api/profile.api.dart:L11`). For the
...
**`Profile`** (`Source: mobile/lib/features/profile/domain/models/profile.dart:L8-L34`),
...
(`Source: mobile/lib/features/profile/domain/models/profile.dart:L24`,
`Source: mobile/lib/features/profile/domain/models/profile.dart:L26`).
...
`Source: mobile/lib/features/profile/domain/models/profile.dart:L31`). This is
the correct contrast to the Recipe model's `copyWith`, whose `inFavorite`
parameter is a documented no-op described in
[`../../../../docs/DATA_MODELS.md`](../../../../docs/DATA_MODELS.md).

**`Preferences`** (`Source: mobile/lib/features/profile/domain/models/preferences.dart:L6-L22`),
annotated `@JsonSerializable()` with `part 'preferences.g.dart';`
(`Source: mobile/lib/features/profile/domain/models/preferences.dart:L5`,
`Source: mobile/lib/features/profile/domain/models/preferences.dart:L3`).

| Field | Type | Default | Source |
|-------|------|---------|--------|
| `dietary` | `List<String>` | `const []` | `preferences.dart:L7` |
| `allergies` | `List<String>` | `const []` | `preferences.dart:L8` |
| `dislikedIngredients` | `List<String>` | `const []` | `preferences.dart:L9` |
| `cookingTime` | `double?` | none (nullable) | `preferences.dart:L10` |
...
|--------|------|---------|--------|
| `GET` | `/api/auth/me` | Fetch the current user's profile | `endpoints.dart:L15` (`ProfileApi.getProfile`, `profile.api.dart:L14-L17`) |
| `PATCH` | `/api/users` | Update the profile (patch body omits null fields) | `endpoints.dart:L16` (`ProfileApi.updateProfile`, `profile.api.dart:L19-L21`) |
| `POST` | `/api/auth/logout` | Server-side logout | `endpoints.dart:L14` (`ProfileApi.logout`, `profile.api.dart:L23-L25`) |
...
(`Source: mobile/lib/features/profile/domain/repositories/profile.repository.dart:L4`,
`Source: mobile/lib/features/profile/domain/repositories/profile.repository.dart:L6`,
`Source: mobile/lib/features/profile/domain/repositories/profile.repository.dart:L8`).
...
- `GetProfileUsecase.call()` instantiates `ProfileRepositiryImpl()` and returns
  `repo.getProfile()`
  (`Source: mobile/lib/features/profile/domain/usecases/get_profile.usecase.dart:L9-L10`).
- `FavoriteRecipesUpdateUsecase.call(dto)` calls
  ...
  (`Source: mobile/lib/features/profile/domain/usecases/favorite_recipes_update.usecase.dart:L13`,
  `Source: mobile/lib/features/profile/domain/usecases/favorite_recipes_update.usecase.dart:L16-L19`).
- `LogoutUsecase.call(context)` logs out server-side, removes the persisted
  ...
  (`Source: mobile/lib/features/profile/domain/usecases/logout.usecase.dart:L19-L28`).
...
- **API base URL.** The backend root comes from `EnvConfig.apiBaseUrl`, a
  ...
  (`Source: mobile/lib/features/profile/data/api/profile.api.dart:L11`).
- **Persisted state.** Hydrated storage is built once during app startup —
  ...
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L16`).
...
(`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L14-L69`),
`ProfileRepositiryImpl`
(`Source: mobile/lib/features/profile/data/repositories/profile.repositiry.dart:L6-L24`),
`ProfileApi`
(`Source: mobile/lib/features/profile/data/api/profile.api.dart:L14-L25`),
and the endpoint constants
(`Source: mobile/lib/core/constants/endpoints.dart:L14-L16`)._
...
- **Load profile.** `ProfileFetched` triggers `GetProfileUsecase`, which calls
  ...
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L18-L22`).
- **Toggle a favorite.** `FavoriteRecipesListUpdated` rebuilds the favorite-id
  ...
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L34-L60`).
  An empty favorites list short-circuits the fetch path
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L24-L32`).
- **Logout.** `Logout` runs `LogoutUsecase`, and `ProfileDataReseted` emits a
  fresh `ProfileState`
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L62-L69`).
...
- **BLoC with persisted state.** `ProfileBloc` extends
  ...
  (`Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L14`,
  `Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L73-L80`,
  `Source: mobile/lib/features/profile/presentation/bloc/profile/profile_bloc.dart:L83-L88`).
...
- **DTO mapping.** `ProfileUpdateDto.toJsonWithoutNullFields()` builds a
  ...
  (`Source: mobile/lib/features/profile/data/dto/profile_update.dto.dart:L22-L38`).
...
(`Source: mobile/lib/features/profile/presentation/widgets/screens/preferences.dart:L10-L16`).
