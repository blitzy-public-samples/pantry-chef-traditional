...
All URLs are built from `Endpoints.apiBaseUrl` (default `http://192.168.2.20:3000/api`). Although `AuthController` declares `@Controller({ path: 'auth', version: '1' })` (Source: `backend/src/auth/auth.controller.ts:L40`), `backend/src/main.ts` never calls `enableVersioning()` (Source: `backend/src/main.ts:L14-L19`), so no `/v1` segment is emitted — the effective paths are `/api/auth/<route>`.
