...
All routes sit under `/api/pantry`. The backend declares `@Controller({ path: 'pantry', version: '1' })`, but the `version: '1'` argument is **inert** because `main.ts` never calls `app.enableVersioning()`, so under the global `api` prefix the routes carry no `/v1/` segment (Source: `mobile/lib/core/constants/endpoints.dart:L92`; `backend/src/main.ts:L10-L35`). Every endpoint is JWT-protected by a class-level guard (Source: `backend/src/pantry/pantry.controller.ts:L42`).
